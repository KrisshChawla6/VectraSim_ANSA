#
# This script improves mesh quality per patch, using ANSA improvement methods
#
# ==========================
...
def isolate_and_fix(shells_to_fix):
    ...
    def has_bad_visible_shells():
        visible_shells = base.CollectEntitiesI(
            DECK, None, SHELL_TYPE, True, filter_visible=True
        )
...
def isolate_problematic_cell(target_shell):
...
def fix_bad_shells_per_area():
    ...
    while bad_shells_remaining:
        ...
        bad_shells_visible, current_visible = isolate_problematic_cell(target_shell)

        # 2. Apply fix to the isolated visible shells
        print(
            f"Fixing {len(bad_shells_visible)} poor-quality shells in isolated group..."
        )
