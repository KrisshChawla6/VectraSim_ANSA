...
from ansa import base, mesh, guitk
...
def shell_join(shell1: base.Entity, shell2: base.Entity):
    ...
    shell1_nodes = shell1.get_entity_values(DECK, ("N1", "N2", "N3", "N4"))
    shell2_nodes = shell2.get_entity_values(DECK, ("N1", "N2", "N3", "N4"))
    if len(shell1_nodes) == 4:
        print(f"Error: Shell {shell1._id} is a QUAD. Please select two TRIA shells to join.")
        return
    if len(shell2_nodes) == 4:
        print(f"Error: Shell {shell2._id} is a QUAD. Please select two TRIA shells to join.")
        return
    ...
    def get_shell_and_node_info(shell: base.Entity) -> dict:
        ...
        shell_thickness = shell.get_entity_values(DECK, ("THIC1", "THIC2", "THIC3", "THIC4"))
        ...
        for node_key, node_entity in shell_entities.items():
            if node_entity is None:
                ...
                node_dict = {"entity": node_entity,
                             "thickness": shell_thickness[f"THIC{node_key.strip('N')}"]}
                node_entities = node_entity.get_entity_values(DECK, ("X", "Y", "Z"))
    ...
    shell1_entities = shell1.get_entity_values(DECK, ("N1", "N2", "N3"))
    shell1_thickness = shell1.get_entity_values(DECK, ("THIC1", "THIC2", "THIC3"))
    shell2_entities = shell2.get_entity_values(DECK, ("N1", "N2", "N3"))
    shell2_thickness = shell2.get_entity_values(DECK, ("THIC1", "THIC2", "THIC3"))
    ...
    for key1, node1 in shell1_entities.items():
        for key2, node2 in shell2_entities.items():
            if node1._id == node2._id:
                common_node = {"node_entity": node1, "thickness": shell1_info[node1]["thickness"]}
    ...
    unique_nodes = [node for node in shell1_entities.values() if node not in common_nodes] + \
    ...
    new_shell, debug_report = base.CreateEntity(DECK, SHELL_TYPE, {"type": "QUAD", "PID": shell1.get_entity_values(DECK, ("EID", "PID"))["PID"],
                                                     "N1": unique_nodes[0], "THIC1": shell1_info[unique_nodes[0]]["thickness"],
                                                     "N2": common_nodes[0], "THIC2": common_nodes_dict[common_nodes[0]]["thickness"],
                                                     "N3": unique_nodes[1], "THIC3": shell2_info[unique_nodes[1]]["thickness"],
                                                     "N4": common_nodes[1], "THIC4": common_nodes_dict[common_nodes[1]]["thickness"]}, debug=ansa.constants.REPORT_ALL)
    print(debug_report)
