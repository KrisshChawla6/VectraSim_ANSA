#
# Main file to execute the geometry agent pipeline.
#
# Note: This script is currently designed to run outside of ANSA. It requires ANSA to export CAD, and will export issues.json after analysis, to be imported again in an ANSA script.
#
# ==========================
...
from . import utils as geom_agent_utils
...
def run_agent(cad_path, mid_path, current_dir: str = "", **kwargs):
    ...
    Vg, Fg, NidG, EidG, ThG = geom_agent_utils.clean_mesh(
        Vg_orig, Fg_orig, NidG_orig, EidG_orig, ThG_orig
    )
    Vm, Fm, NidM, EidM, ThM = geom_agent_utils.clean_mesh(
        Vm_orig, Fm_orig, NidM_orig, EidM_orig, ThM_orig
    )
    ...
    # Triangulate
    tri_G, tri2eid_G = geom_agent_utils.triangulate(Fg, EidG)
    tri_M, tri2eid_M = geom_agent_utils.triangulate(Fm, EidM)
    ...
    Vg, tri_G, NidG, G_removed_nodes, ThG = geom_agent_utils.remove_unreferenced(
        Vg, tri_G, NidG, ThG
    )
    Vm, tri_M, NidM, M_removed_nodes, ThM = geom_agent_utils.remove_unreferenced(
        Vm, tri_M, NidM, ThM
    )
    ...
    elem_G_w_removed_nodes = [
        i
        for i, face in enumerate(Fg_orig)
        if any(node_idx in face for node_idx in G_removed_nodes_idx)
    ]
    elem_M_w_removed_nodes = [
        i
        for i, face in enumerate(Fm_orig)
        if any(node_idx in face for node_idx in M_removed_nodes_idx)
    ]
    ...
    tol_thickness = kwargs.get(
        "tol_thickness", 0.2
    )  # 0.2 relative tolerance for thickness
    issues = diagnose(
        results,
    ...
    # Save issues to json file, to be imported again in an ANSA script
