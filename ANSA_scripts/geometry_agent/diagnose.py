...
def diagnose(results,
    ...
    M_nodes_far = [node_id for node_id in M_nodes_far if rayhit_t[nodeid_to_idx.get(node_id, 0)] < 2 or (rayhit_t[nodeid_to_idx.get(node_id, 0)] == 2) and ray_t[nodeid_to_idx.get(node_id, 0)] > 100 * thickness_M[nodeid_to_idx.get(node_id, 0)]]
    ...
    if closest_node_M is not None:
        thickness_G = np.array([tgt_t[nodeid_to_idx.get(nid, 0)] for nid in closest_node_M])
